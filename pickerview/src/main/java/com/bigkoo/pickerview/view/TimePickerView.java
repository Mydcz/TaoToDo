...
public class TimePickerView extends BasePickerView implements View.OnClickListener {
    ...
    public TimePickerView(PickerOptions pickerOptions, AppType type) {
        super(pickerOptions.context);

        mPickerOptions = pickerOptions;
        initView(pickerOptions.context, type);
    }

    private void initView(Context context, AppType type) {
        setDialogOutSideCancelable();
        initViews();
        initAnim();
        setOnKyeClickListener((keyCode, event) -> {
            if (keyCode == KeyEvent.KEYCODE_BACK) {

                if (mPickerOptions.cancelListener != null) {
                    mPickerOptions.cancelListener.onClick(clickView);
                }
            }
        });

        if (mPickerOptions.customListener == null) {
            if (type == AppType.WatchFitPro) {
                LayoutInflater.from(context).inflate(R.layout.watchfitpro_pickerview_time, contentContainer);
            } else {
